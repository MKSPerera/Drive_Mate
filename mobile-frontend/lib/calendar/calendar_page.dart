...
/// Calendar page that displays the user's availability and accepted jobs.
/// Allows users to view and manage their schedule.
...
class _CalendarPageState extends State<CalendarPage> {
  final CalendarFormat _calendarFormat =
      CalendarFormat.month; // Calendar format
  DateTime _focusedDay = DateTime.now(); // Currently focused day
  DateTime? _selectedDay; // Currently selected day
  List<DateTimeRange> busyDays = []; // List of busy days
  final JobService _jobService =
      JobService(); // Service for job-related API calls
  ...
  DateTimeRange? highlightedRange; // Range to highlight on the calendar
  final int _selectedTabIndex = 1; // Set to 1 for Calendar tab
  List<DateTimeRange> acceptedJobRanges = []; // List of accepted job ranges

  @override
  void initState() {
    super.initState();
    _loadAvailability(); // Load availability on initialization
  }

  /// Loads availability and accepted jobs from the backend
  ...
  CalendarBuilders get calendarBuilders => CalendarBuilders(
        rangeHighlightBuilder: (context, day, isWithinRange) {
          ...
          bool isWithinAcceptedJob = acceptedJobRanges.any((range) =>
              (day.isAtSameMomentAs(range.start.subtract(Duration(days: 1))) ||
